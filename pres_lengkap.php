...
<section id="pengumuman">
    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header">List Prestasi
...
$sql_tampil=mysql_query("SELECT * FROM prestasi ORDER BY tanggal_pres DESC limit ".$posisi.",".$batas."");
$cek_row=mysql_num_rows($sql_tampil);
if($cek_row>0){
$no=0;
  while( $data1=mysql_fetch_array($sql_tampil)){
  
$tgl=$data1['tanggal_pres'];
...
$isi=$data1['isi_pres'];
    ?>
        ...
        <div class="row">
            <div class="col-md-4">
                <a href="#">
                    <img  class="img-responsive" src="gambar2/<?php echo $data1['gambar'];  ?>" alt="">
                </a>
            </div>
            <div class="col-md-7">
                <h3><?php echo $data1['judul_pres']; ?></h3>
                <small><?php echo $hari[$day].','.$result; ?></small>
                <p></p><?php echo substr($isi, 0,200); ?></p>
              <button  type="button" data-id="<?php echo $data['id_pres']; ?>" class="btn" style="background-color: #3498DC; color: white;" data-toggle="popover" data-animation='pop' title="<?php echo $data['judul_pres']; ?>" data-html="true" data-content="<img src='gambar2/<?php echo $data['gambar']; ?>' style='margin-bottom:10px;' class='img-responsive'/><?php echo $data['isi_pres']; ?>">Lanjutkan <span class="glyphicon glyphicon-circle-arrow-right"></span></button>
            </div>
        </div>
        <!-- /.row -->
...
$jml_data=mysql_num_rows(mysql_query("SELECT * FROM prestasi"));
...
echo $prev . $nmr . $next;
   ...
   <?php }else{ ?>
    <div class="alert alert-info" style="margin-right: 20px; margin-left: 20px;" role="alert">Prestasi tidak ada</div>
    <?php }?>
      </div>
